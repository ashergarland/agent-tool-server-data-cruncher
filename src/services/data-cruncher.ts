...
export class DataCruncherService {
  ...
  private async readableFile(filePath: string): Promise<string> {
    ...
    const candidate = await realpath(
      isAbsolute(filePath) ? filePath : resolve(root, filePath),
    ).catch(() => {
    ...
    if (!(await stat(candidate)).isFile())
      throw badRequest('File path must refer to a regular file');
    return candidate;
  }
  ...
  public async ripgrep(
    ...
    try {
      ...
      if (failure.code === 'ENOENT')
        throw new Error('ripgrep is not installed', { cause: failure });
